...
class ZookeeperMockClient {
    ...
    create(path, data, acls, mode, callback) {
        ...
        return this._mock._withRandomDelay(
            () => this._create(path, data, acls, mode, callback));
    }
    ...
    setData(path, data, version, callback) {
        ...
        this._validatePath(path);
        return this._mock._withRandomDelay(
            () => this._setData(path, data, version, callback));
    }

    _setData(path, data, version, callback) {
    ...
    getData(path, watcher, callback) {
        ...
        this._validatePath(path);
        return this._mock._withRandomDelay(
            () => this._getData(path, watcher, callback));
    }

    _getData(path, watcher, callback) {
        const { parent, baseName, err } = this._getZNode(path);
        if (err) {
            return process.nextTick(() => callback(err));
        }
    ...
    getChildren(path, watcher, callback) {
        ...
        this._validatePath(path);
        return this._mock._withRandomDelay(
            () => this._getChildren(path, watcher, callback));
    }

    _getChildren(path, watcher, callback) {
        const children = [];
        const { parent, baseName, err } = this._getZNode(path);
        if (err) {
            return process.nextTick(() => callback(err));
    ...
    mkdirp(path, data, acls, mode, callback) {
        ...
        this._validatePath(path);
        return this._mock._withRandomDelay(
            () => this._mkdirp(path, data, acls, mode, callback));
    }

    _mkdirp(path, data, acls, mode, callback) {
        let currentPath = '';
    ...
    exists(path, watcher, callback) {
        ...
        this._validatePath(path);
        return this._mock._withRandomDelay(
            () => this._exists(path, watcher, callback));
    }

    _exists(path, watcher, callback) {
        const { parent, baseName, err } = this._getZNode(path);
        if (err) {
            if (err.name === 'NO_NODE') {
                return process.nextTick(
    ...
    remove(path, version, callback) {
        ...
        this._validatePath(path);
        return this._mock._withRandomDelay(
            () => this._remove(path, version, callback));
    }

    _remove(path, version, callback) {
        const arr = path.split('/');
...
class ZookeeperMock {
    constructor(config) {
        this._doLog = false;
        this._maxRandomDelay = 0;
        this._minRandomDelay = 0;
        if (config !== undefined) {
            if (config.doLog) {
                this._doLog = config.doLog;
            }
            if (config.maxRandomDelay) {
                this._maxRandomDelay = config.maxRandomDelay;
            }
            if (config.minRandomDelay) {
                this._minRandomDelay = config.minRandomDelay;
            }
        }
        this._zkState = {};
        this._resetState();
    ...
    _withRandomDelay(cb) {
        if (this._maxRandomDelay > 0) {
            const diff = this._maxRandomDelay - this._minRandomDelay;
            setTimeout(cb, this._minRandomDelay + Math.random() * diff);
        } else {
            cb();
        }
    }
