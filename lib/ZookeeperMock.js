...
class ZookeeperMock {
    ...
    setData(path, data, version, callback) {
        ...
        prev.children[prevName].data = data;
        // trigger watcher
        prev.children[prevName].emitter.emit('watcher', {
    ...
    exists(path, watcher, callback) {
        ...
        if (prev && !prev.children[prevName]) {
    ...
    _remove(path) {
        function traverseAndRemove(arr, idx, node) {
            const name = arr[idx];
            ...
            if (idx === (arr.length - 1)) {
                ...
                // eslint-disable-next-line
                idx++;
                return traverseAndRemove(arr, idx, node.children[name]);
            }
        }

        const arr = path.split('/');
        const result = traverseAndRemove(arr, 0, _zkState);
